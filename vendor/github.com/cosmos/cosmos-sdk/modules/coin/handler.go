...
func (h Handler) sendTx(ctx sdk.Context, store state.SimpleDB,
	...
	for _, in := range send.Inputs {
		_, err = ChangeCoins(store, in.Address, in.Coins.Negative(), in.Post)
		if err != nil {
			return res, err
		}
	...
	for _, out := range send.Outputs {
		...
		_, err = ChangeCoins(store, out.Address, out.Coins, out.Post)
		if err != nil {
			return res, err
		}
...
func (Handler) checkSendTx(ctx sdk.Context, store state.SimpleDB, send SendTx) error {
	...
	for _, in := range send.Inputs {
		_, err := CheckCoins(store, in.Address, in.Coins.Negative(), in.Post)
		if err != nil {
			return err
		}
	}
